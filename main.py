...
class MyGame(arcade.Window):
    ...
    def on_draw(self):
        ...
        if self.game_state == game_state.GameState.ROUND_DONE:
            ...
            arcade.draw_text("Appuyer sur espace pour rejouer", start_x=150, start_y=500, font_size=30,color=arcade.color.LIGHT_BLUE)
        ...
        if self.game_state == game_state.GameState.GAME_OVER:
            if self.user_points == 3:
                arcade.draw_text("Vous avez gagner la partie", start_x=150, start_y=510, font_size=35, color=arcade.color.LIGHT_BLUE)

            if self.bot_points == 3:
                arcade.draw_text("Vous avez perdu la partie", start_x=150, start_y=500, font_size=30, color=arcade.color.LIGHT_BLUE)

            arcade.draw_text("Appuyer sur espace pour refaire une partie", start_x=55, start_y=450, font_size=25, color=arcade.color.LIGHT_BLUE)
    ...
    def on_update(self, delta_time):
        if self.bot_attack.number_of_swaps == 6:
            ...
            self.bot_attack = attack_animations.AttackAnimations(attack_type=random.choice(CHOICES), pos_x=625,pos_y=200)
            self.update_bot_attack = False
    ...
    def not_moving_image(self):
        arcade.draw_text(text = "Pointage User: %d"%self.user_points, start_x=100, start_y=100, font_size=15, color=arcade.color.LIGHT_BLUE)
        arcade.draw_text(text = "Pointage Bot: %d"%self.bot_points, start_x=550, start_y=100, font_size=15, color=arcade.color.LIGHT_BLUE)
